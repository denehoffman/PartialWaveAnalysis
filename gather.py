#!/usr/bin/env python3
"""
gather_fits.py: This program collects data from AmpTools fits and stores them in a
    single location. Run it without arguments for a more detailed description of its
    inputs.
...
def gather(output_dir, config_file, bootstrap, n_iterations):
    ...
    headers = [] # generate a header with the column labels
    amplitudes = []
    with open(config_file, 'r') as config: # collect amplitudes from config
        for line in config.readlines():
            if line.startswith("amplitude"):
                amplitudes.append(line.split()[1].strip())
    for wave_name in amplitudes: # Intensity headers
    ...
    for wave_name in amplitudes: # Real/Imaginary parts of amplitude headers
        wave_parts = wave_name.split("::")
        if wave_parts[1].endswith("Re"):
            headers.append(wave_parts[2] + "_re")
            headers.append(wave_parts[2] + "_im")
    amp_pairs = combinations(amplitudes, 2) # gets all unique 2-element combinations
    for amp_pair in amp_pairs: # Phase difference headers
    ...
    headers.append("total_intensity") # headers for total intensity
    headers.append("total_intensity_err")
    headers.append("total_intensity_AC")
    headers.append("total_intensity_err_AC")
    headers.append("likelihood") # headers for likelihood
    if bootstrap: # we have a different file name for bootstrap runs
    ...
    with open(output_dir / output_file_name, 'w') as out_file:
        ...
        bin_converged_total = np.zeros_like(bin_dirs) # tally for converged iterations
        bin_total_iterations = np.zeros_like(bin_dirs) # tally for iterations
        for bin_dir in tqdm(bin_dirs): # for each bin (loading bar)
            bin_num_string = bin_dir.name
            iter_dirs = [ # list of iteration directories in the given bin
                iterdir for iterdir in bin_dir.glob("*")
                if iterdir.is_dir() and int(iterdir.name) < n_iterations
            ] # limit this list to the number of iterations we just ran
            for iteration_dir in iter_dirs:
                ...
                if bootstrap:
                    ...
                    fit_results = iteration_dir / (config_file.stem +
                                                   "::fit_results.txt")
                bin_total_iterations[int(bin_num_string)] += 1 # tally up an iteration
                converged = "U"
                if fit_results.exists():
                    with open(fit_results, "r") as fit_results_reader:
                        line = fit_results_reader.readline()
                        if line != "": # for converged letter in fit file
                            converged = line.split("\t")[2]
                    if converged == "C": # tally up a converged iteration
                        bin_converged_total[int(bin_num_string)] += 1
                    with open(fit_results) as fit_reader:
                        out_file.write(fit_reader.read())
                        # write iteration contents to output file
        print("Convergence Results:") # nice colored output to show tallies
        for i, bin_converged_num in enumerate(bin_converged_total):
            ...
            if percent_converged == 0:
                ...
                color = Fore.GREEN
            print((f"{color}Bin {i}: {bin_converged_total[i]}/{bin_total_iterations[i]}"
                   f"\t{Fore.WHITE}"), end='')
        print()
