...
for bin_num in np.arange(args.n):
    ...
    for bin_file in bin_files:
        ...
        with open(config_path) as config:
            ...
            if args.data != None:
                data_path_AMO = [filepath for filepath in data_dir.glob("*.root") if "AMO" in filepath.name][0]
                data_path_000 = [filepath for filepath in data_dir.glob("*.root") if "PARA_0" in filepath.name][0]
                data_path_045 = [filepath for filepath in data_dir.glob("*.root") if "PERP_45" in filepath.name][0]
                data_path_090 = [filepath for filepath in data_dir.glob("*.root") if "PERP_90" in filepath.name][0]
                data_path_135 = [filepath for filepath in data_dir.glob("*.root") if "PARA_135" in filepath.name][0]
            ...
            if args.background != None:
                background_path_AMO = [filepath for filepath in background_dir.glob("*.root") if "AMO" in filepath.name][0]
                background_path_000 = [filepath for filepath in background_dir.glob("*.root") if "PARA_0" in filepath.name][0]
                background_path_045 = [filepath for filepath in background_dir.glob("*.root") if "PERP_45" in filepath.name][0]
                background_path_090 = [filepath for filepath in background_dir.glob("*.root") if "PERP_90" in filepath.name][0]
                background_path_135 = [filepath for filepath in background_dir.glob("*.root") if "PARA_135" in filepath.name][0]
