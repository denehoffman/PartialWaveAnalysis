...
parser.add_argument("-d",
                    "--data",
                    action='store_true',
                    help="plot true data histogram behind intensities")
parser.add_argument("-b",
                    "--background",
                    action='store_true',
                    help="plot background histogram behind intensities")
parser.add_argument("-s",
                    "--subtracted",
                    action='store_true',
                    help="plot background-subtracted data behind intensities")
parser.add_argument(
    "-l",
    "--label",
    default="K_SK_S",
...
parser_group.add_argument("--bins",
                          type=int,
                          help="set number of bins for data")
parser_group.add_argument(
    "--optimize",
    action='store_true',
    help="optimize data binning (experimental, currently doesn't work)")
...
pdf = matplotlib.backends.backend_pdf.PdfPages(
    f"figs_{fit_results.stem.split('::')[0]}.pdf")
...
bin_df['Centers'] = np.linspace(bin_df['mass'].iloc[0] + bin_width / 2,
...
bin_dirs = [
    bin_dir for bin_dir in fit_results.parent.iterdir() if bin_dir.is_dir()
]
M2s = []
M2s_bkg = []
M2s_sub = []
...
for bin_dir in tqdm(bin_dirs):
    if args.data or args.background or args.subtracted:
        data_files = [
            data_file for data_file in bin_dir.iterdir()
        ...
        for data_file in data_files:
            with uproot.open(data_file) as df:
                ...
                ]  #ignore first particle, which is the proton
                M2s += [
                    P4[0]**2 - np.sum(np.power(P4[1:], 2))
                    for P4 in Final_State_P4
                ]
                M2s_sub += [
                    P4[0]**2 - np.sum(np.power(P4[1:], 2))
                    for P4 in Final_State_P4
    ...
    if args.background or args.subtracted:
        bkg_files = [
            bkg_file for bkg_file in bin_dir.iterdir()
...
if args.optimize:
    print(
        "WARNING: Plotting data with an optimized binning algorithm. This is an experimental feature!"
    )
...
for wave in waves_sorted:
    ...
    if args.data:
        plt.hist(invariant_mass,
                 bins=nbins,
                 range=(bin_df['mass'].iloc[0], bin_df['mass'].iloc[-1]),
                 weights=weights,
                 color=data_colors[0],
                 label="Data",
                 fill=False,
    ...
    if wave_pos in amplitudes:
        print("+e", end='\t')
        plt.errorbar(bin_df['Centers'].iloc[df_filtered['Bin']],
                     df_filtered[wave_pos + ac_tag],
                     yerr=df_filtered[wave_pos + "_err" + ac_tag],
                     elinewidth=0.5,
                     fmt='o',
                     color='r',
                     label=r'$+\epsilon$')
    ...
    if wave_neg in amplitudes:
        ...
        plt.errorbar(bin_df['Centers'].iloc[df_filtered['Bin']],
                     ...
                     yerr=df_filtered[wave_neg + "_err" + ac_tag],
                     elinewidth=0.5,
                     fmt='o',
                     color='b',
                     label=r'$-\epsilon$')
    ...
    plt.errorbar(bin_df['Centers'].iloc[df_filtered['Bin']],
                 ...
                 yerr=df_filtered['total_intensity_err' + ac_tag_total],
                 elinewidth=0.5,
                 fmt='none',
                 color='k')
    plt.hist(bin_df['Centers'].iloc[df_filtered['Bin']],
             bins=len(bin_df),
             range=(bin_df['mass'].iloc[0], bin_df['mass'].iloc[-1]),
             weights=df_filtered['total_intensity' + ac_tag_total],
             fill=False,
             histtype='step',
             color='k',
...
for phase_diff in phase_diffs:
    fig, ax = plt.subplots()
    ax2 = ax.twinx()
    ax.errorbar(bin_df['Centers'].iloc[df_filtered['Bin']],
                df_filtered['total_intensity' + ac_tag_total],
                yerr=df_filtered['total_intensity_err' + ac_tag_total],
                elinewidth=0.5,
                fmt='none',
                color='k')
    ax.hist(bin_df['Centers'].iloc[df_filtered['Bin']],
            bins=len(bin_df),
            range=(bin_df['mass'].iloc[0], bin_df['mass'].iloc[-1]),
            weights=df_filtered['total_intensity' + ac_tag_total],
            fill=False,
