...
pdf = matplotlib.backends.backend_pdf.PdfPages(
    f"amps_{fit_results.stem.split('::')[0]}.pdf")
...
amplitudes = [
    column[:-3] for column in df.columns.to_list() if column.endswith('_re')
]
print(amplitudes)
...
for bin_n in range(len(bin_df)):
    ...
    for i, wave in enumerate(waves_sorted):
        ...
        if wave_pos in amplitudes:
            ...
            x_range = max(max(abs(xmax), abs(xmin)), 10)
            ...
            try:
                ...
                values = np.vstack([
                    df_conv.loc[:, wave_pos + '_re'],
                    df_conv.loc[:, wave_pos + '_im']
                ])
                ...
                pass
            axes[0, i].scatter(df_conv.loc[:, wave_pos + '_re'],
                               df_conv.loc[:, wave_pos + '_im'],
                               color='k',
                               marker=',')
            axes[0, i].scatter(df_filtered[wave_pos + '_re'].iloc[bin_n],
                               df_filtered[wave_pos + '_im'].iloc[bin_n],
                               color='r',
                               marker=',')
        ...
        if wave_neg in amplitudes:
            ...
            try:
                ...
                positions = np.vstack([xx.ravel(), yy.ravel()])
                values = np.vstack([
                    df_conv.loc[:, wave_neg + '_re'],
                    df_conv.loc[:, wave_neg + '_im']
                ])
                kernel = st.gaussian_kde(values)
                f = np.reshape(kernel(positions).T, xx.shape)
                if args.contours:
                ...
                pass
            axes[1, i].scatter(df_conv.loc[:, wave_neg + '_re'],
                               df_conv.loc[:, wave_neg + '_im'],
                               color='k',
                               marker=',')
            axes[1, i].scatter(df_filtered[wave_neg + '_re'].iloc[bin_n],
                               df_filtered[wave_neg + '_im'].iloc[bin_n],
                               color='r',
                               marker=',')
