...
def run_fit(bin_iterations_seed_reaction_bootstrap_tuple):
    """Runs an iteration of an AmpTools fit on a given bin.
    :param bin_iterations_seed_reaction_bootstrap_tuple: tuple(int, int, int, str, bool), contains the bin number, iteration number, seed, reaction name, and bootstrap status

    :rtype: None
    :return: None
    """
    bin_number, iteration, seed, reaction, bootstrap = bin_iterations_seed_reaction_bootstrap_tuple # get info for this run
    ...
    if bootstrap:
        config_file = [f for f in Path(".").glob(f"*_{bin_number}_bootstrap.cfg")][0] # locate the (bootstrap) config file
    else:
    ...
    if bootstrap:
        fit_output_destination = Path(fit_output.stem + f"::{status}.bootstrap").resolve()
    else:
...
bin_iterations_seed_reaction_bootstrap_tuple = [(i, j, seeds[j], reaction, args.bootstrap) for i in range(n_bins) for j in range(args.iterations)]
...
if args.slurm:
    for tup in tqdm(bin_iterations_seed_reaction_bootstrap_tuple):
        bin_number, iteration, seed, reaction, bootstrap = tup
        bin_n, it_n, _, rxn, _ = tup
        log_out = log_dir / f"{rxn}_{bin_n}_{it_n}_SLURM.out"
        log_err = log_dir / f"{rxn}_{bin_n}_{it_n}_SLURM.err"
        os.system(f"sbatch --job-name={rxn}_{bin_n}_{it_n} --ntasks={threads} --partition={queue} --mem={memory} --time=30:00 --output={str(log_out)} --error={str(log_err)} --quiet ../run_fit_slurm.csh {bin_number} {iteration} {seed} {reaction} {str(log_dir)} {str(bootstrap)}")
        time.sleep(1)
    ...
    with Pool(processes=args.processes) as pool: # create a multiprocessing pool
        res = list(tqdm(pool.imap(run_fit, bin_iterations_seed_reaction_bootstrap_tuple), total=args.iterations * n_bins)) # imap(x, y) spawns processes which run a method x(y)
